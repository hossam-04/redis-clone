...
## Current state
**Week 1 — done.** Real `redis-cli` connects and runs `PING`, `ECHO`, `SET`,
and `GET` against this server unmodified. Read path, write path, dispatch, and
a mutex-guarded store are all in place; 20 concurrent clients run clean under
`-race`.
...
## Next up
Week 2: key expiry (`EX` / `PX`) and LRU eviction. First design question is
